...
### Paper-faithful 3-regular TFIM instances

The paper benchmark ensemble is generated by:

```text
H(Gamma) = sum_(i,j in E) Z_i Z_j - Gamma sum_i X_i
```

where `E` is a simple 3-regular graph. The tracked catalog is
`instances/paper_tfim_3regular_catalog.json`; generated Hamiltonians are
local artifacts and are ignored by Git:

```bash
python3 experiments/paper_tfim_3regular.py catalog \
  instances/paper_tfim_3regular_catalog.json \
  generated_instances/paper_tfim_3regular
```

Each generated instance contains `instance.json`, `H_fixed.txt`, and
`H_gamma.txt`. The catalog's `gamma_values` are reference values from the
papers only; any Gamma can be used with the split Hamiltonian.
