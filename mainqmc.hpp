...
double measure_Hdiag_Fint2(){
    ...
    if (q == 0) {
         ...
         *     + exp[-run_beta [E_0,...,E_{q-1}]] )
         */
        tot_num += (ddprefac * Ezq + ddprev) * (run_beta * run_beta / 8.0);

        /*
         * Correction term:
         *
         *   - sum_{r=0}^q
         *       exp[-run_beta/2 [E_0,...,E_r]]
         *       sum_{i=r+1}^q
         *          (i-r) exp[-run_beta/2 [E_r,...,E_q,E_i]]
         */
...
double measure_Hdiag_k2Int(){
    ...
    if(q == 0){
      ...
      return run_beta * (d->z[0]) * (d->z[0]) / 24.0;
    }
    ...
    result += run_beta * run_beta * run_beta * Ezq * Ezq / 24.0;
    result += (ddprev / ddprefac) * (run_beta * run_beta * run_beta * Ezq / 24.0);
...
double compute_derived_observable(int n){ // we compute the derived observables
	...
	case 5 : R = mean_O[Nobservables+15] - run_beta*(run_beta*mean_O[Nobservables+2])*(run_beta*mean_O[Nobservables+2])/24.0; break;
	case 6 : R = (mean_O[Nobservables+8] - run_beta*mean_O[Nobservables+2]*mean_O[Nobservables+2]) / 2.0 /  (mean_O[Nobservables+9] - (run_beta*mean_O[Nobservables+2])*(run_beta*mean_O[Nobservables+2])/8); break;
	case 7 : R = 2 * (mean_O[Nobservables+9] - (run_beta*mean_O[Nobservables+2])*(run_beta*mean_O[Nobservables+2])/8) / (mean_O[Nobservables+15] - run_beta*(run_beta*mean_O[Nobservables+2])*(run_beta*mean_O[Nobservables+2])/24.0); break;
	}
	return R;
}
