...
int main(int argc, char *argv[]) {
  ...
  double sgn_variance = 0;
  double over_bins_sum_cov[N_all_observables] = {0};
  for (i = 0; i < Nbins; i++)
    sgn_mean += bin_mean_sgn[i];
  sgn_mean /= Nbins;
  for (i = 0; i < Nbins; i++)
    sgn_variance += (bin_mean_sgn[i] - sgn_mean) * (bin_mean_sgn[i] - sgn_mean);
  sgn_variance /= (Nbins * (Nbins - 1));
  ...
  if (qmax_achieved)
    std::cout << std::endl
              << "Warning: qmax = " << qmax
              << " was achieved. The results may be incorrect. The qmax "
                 "parameter should be increased."
              << std::endl;
  for (i = 0; i < Ncycles; i++)
    if (!cycles_used[i])
      std::cout << "Warning: cycle No. " << i << " of length " << cycle_len[i]
                << " was not used" << std::endl;
  std::cout << "mean(q) = " << meanq / measurements << std::endl;
  std::cout << "max(q) = " << maxq << std::endl;
  for (k = 0; k < N_all_observables; k++)
    if (valid_observable[k]) {
      ...
      for (i = 0; i < Nbins; i++)
        over_bins_sum[k] +=
            (bin_mean[k][i] - Rsum[k]) * (bin_mean[k][i] - Rsum[k]);
      over_bins_sum[k] /= (Nbins * (Nbins - 1));
      for (i = 0; i < Nbins; i++)
        over_bins_sum_cov[k] +=
            (bin_mean[k][i] - Rsum[k]) * (bin_mean_sgn[i] - sgn_mean);
      over_bins_sum_cov[k] /= (Nbins * (Nbins - 1));
      mean_O[k] =
          Rsum[k] / sgn_mean * (1 + sgn_variance / sgn_mean / sgn_mean) -
          over_bins_sum_cov[k] / sgn_mean / sgn_mean;
      stdev_O[k] = fabs(Rsum[k] / sgn_mean) *
                   sqrt(over_bins_sum[k] / Rsum[k] / Rsum[k] +
                        sgn_variance / sgn_mean / sgn_mean -
                        2 * over_bins_sum_cov[k] / Rsum[k] / sgn_mean);
      std::cout << "mean(O) = " << mean_O[k] << std::endl;
      std::cout << "std.dev.(O) = " << stdev_O[k] << std::endl;
    }
  double mean_derived_O[N_derived_observables],
      stdev_derived_O[N_derived_observables],
      jackknife_O[N_derived_observables], jackknife_sum[N_derived_observables],
      sgn_meanJ, sgn_varianceJ;
  for (o = 0; o < N_derived_observables; o++)
    if (valid_derived_observable(o)) {
      mean_derived_O[o] = compute_derived_observable(o);
      jackknife_sum[o] = 0;
    }
  for (j = 0; j < Nbins; j++) {
    ...
    for (i = 0; i < Nbins; i++)
      if (i != j)
        sgn_meanJ += bin_mean_sgn[i];
    sgn_meanJ /= (Nbins - 1);
    for (i = 0; i < Nbins; i++)
      if (i != j)
        sgn_varianceJ +=
            (bin_mean_sgn[i] - sgn_meanJ) * (bin_mean_sgn[i] - sgn_meanJ);
    sgn_varianceJ /= ((Nbins - 1) * (Nbins - 2));
    for (k = 0; k < N_all_observables; k++)
      if (valid_observable[k]) {
        ...
        for (i = 0; i < Nbins; i++)
          if (i != j)
            over_bins_sum_cov[k] +=
                (bin_mean[k][i] - Rsum[k]) * (bin_mean_sgn[i] - sgn_meanJ);
        over_bins_sum_cov[k] /= ((Nbins - 1) * (Nbins - 2));
        mean_O[k] =
            Rsum[k] / sgn_meanJ * (1 + sgn_varianceJ / sgn_meanJ / sgn_meanJ) -
            over_bins_sum_cov[k] / sgn_meanJ / sgn_meanJ;
      }
    for (o = 0; o < N_derived_observables; o++)
      if (valid_derived_observable(o)) {
        jackknife_O[o] = compute_derived_observable(o);
        jackknife_sum[o] += (jackknife_O[o] - mean_derived_O[o]) *
                            (jackknife_O[o] - mean_derived_O[o]);
      }
  }
  for (o = 0; o < N_derived_observables; o++)
    if (valid_derived_observable(o))
  ...
  for (o = 0; o < N_derived_observables; o++)
    if (valid_derived_observable(o)) {
      std::cout << "Derived observable: " << name_of_derived_observable(o)
                << std::endl;
      std::cout << "mean(O) = " << mean_derived_O[o] << std::endl;
      std::cout << "std.dev.(O) = " << stdev_derived_O[o] << std::endl;
    }
  divdiff_clear_up();
  std::cout << "wall-clock cpu time = " << get_cpu_time() - start_time
            << " seconds" << std::endl;
  return 0;
}
