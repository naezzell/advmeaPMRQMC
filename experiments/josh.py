...
from utils.pauli_manipulations import PauliTerm, PauliH, PauliU
from utils.ioscripts import make_all_stand_param_fstr, make_no_stand_param_fstr

import joshutils as josh
...
def main(
    ...
    h = josh.random_hermitian(4)
    # with open(f"{dir_name}/H.txt", "w") as f:
    # f.write(h.to_pmr_str())
    josh.write_as_paulis(f"{dir_name}/H.txt", h)
    ...
    # f.write(b.to_pmr_str())
    a = josh.random_hermitian(4)
    b = josh.random_hermitian(4)
    josh.write_as_paulis(f"{dir_name}/A.txt", a)
    josh.write_as_paulis(f"{dir_name}/B.txt", b)
    # ==============================================
    # Running PMR QMC
    # ==============================================
    # param_str = make_all_stand_param_fstr(beta, tau, Tsteps, steps, stepsPerMeasurement, 0, save, restart)
    param_str = make_no_stand_param_fstr(
        beta, tau, Tsteps, steps, stepsPerMeasurement, 0, save, restart
    )
...
if __name__ == "__main__":
    ...
    main(
        ...
        Tsteps=Tsteps,
        steps=steps,
        stepsPerMeasurement=stepsPerMeasurement,
        save=save,
        restart=restart,
    )
# %%
